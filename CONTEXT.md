...
**Session**: A single playthrough of the game between two players, from lobby creation to victory. Identified by a unique session ID embedded in the Invite Link.
...
**Invite Link**: A URL containing the Host's Offer Code in the URL hash. Shared out-of-band (e.g. WhatsApp, text). Never touches a server.
_Avoid_: Game link, share link, join link

**Offer Code**: The base64-encoded WebRTC SDP offer generated by the Host and embedded in the Invite Link.
_Avoid_: Invite code, connection code

**Answer Code**: The base64-encoded WebRTC SDP answer generated by the Guest and shared back to the Host to complete the connection.
_Avoid_: Response code, reply code
...
**Event Roll**: The result of the symbol die (Bandit, Trade, Festival, Harvest, or Event). Processed before the Production Roll; if Bandit is rolled, it replaces the Production Roll.
